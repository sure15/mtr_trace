...
iplise=(219.141.136.12 202.106.50.1 221.179.155.161 202.96.209.133 110.242.68.3 211.136.112.200 58.60.188.222 210.21.196.6 120.196.165.24)
			...
			if [ $? == 0 ];then
				...
				if [ $? == 0 ];then
					...
					if [ $? == 0 ];then
						...
						grep -q "210\." /root/traceroute_testlog
						if [ $? == 0 ];then
							echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;33m联通9929\033[0m"
						else
							echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;33m联通169\033[0m"
